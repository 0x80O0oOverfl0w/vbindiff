...
const short  leftMar  = 16;     // Starting column of hex display
const short  leftMar2 = 91;     // Starting column of ASCII display

const int  lineWidth = 24;      // Number of bytes displayed per line
...
//    cerr << i << '\n';
    char*  str = buf2;
    str +=
      // Write hex address for current line
      sprintf(str, "%04X %04X %04X:", Word(lineOffset>>32), Word(lineOffset>>16), Word(lineOffset&0xFFFF));
    ...
    if (index < 0) 
    {
      index = 0; // in case nothing was printed in this line
    }
    ...
    if (diffs)
      for (j = 0; j < lineLength; j++)
        if (diffs->data->line[i][j]) {
          // Highlight hex differences
          win.putAttribs(j*3 + leftMar  + (j>7) + (j>15), i+1, cFileDiff, 2);
          // Highlight ASCII differences
          win.putAttribs(j   + leftMar2 + (j>7) + (j>15), i+1, cFileDiff,1);
        }
